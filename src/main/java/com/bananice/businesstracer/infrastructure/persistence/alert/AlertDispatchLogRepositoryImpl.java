...
public class AlertDispatchLogRepositoryImpl implements AlertDispatchLogRepository {
    ...
    public void save(AlertDispatchLog alertDispatchLog) {
        ...
        po.setChannelCode(
                alertDispatchLog.getChannelId() == null ? null : String.valueOf(alertDispatchLog.getChannelId()));
        po.setDispatchStatus(
                alertDispatchLog.getStatus() == null
                        ? null
                        : alertDispatchLog.getStatus().name());
    ...
    public List<AlertDispatchLog> findByEventId(Long eventId) {
        ...
        return alertDispatchLogMapper.selectList(query).stream()
                .map(this::toDomain)
