    ...
    public void scanAndPublishFlowStuck() {
        ...
        for (FlowLog flowLog : stuckInProgressFlows) {
            try {
                ...
                log.error(
                        "Failed to open/update flow-stuck alert for flowCode={}, businessId={}",
                        flowLog.getFlowCode(),
                        flowLog.getBusinessId(),
                        e);
            }
        }
        ...
        for (FlowLog flowLog : staleFlows) {
            if (!TraceStatus.IN_PROGRESS.getValue().equalsIgnoreCase(flowLog.getStatus())) {
                try {
                    alertEvaluateService.closeFlowStuckByStatus(
                            flowLog.getFlowCode(), flowLog.getBusinessId(), flowLog.getStatus(), null);
                } catch (Exception e) {
                    log.error(
                            "Failed to close flow-stuck alert for flowCode={}, businessId={}, status={}",
                            flowLog.getFlowCode(),
                            flowLog.getBusinessId(),
                            flowLog.getStatus(),
                            e);
